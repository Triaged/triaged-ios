...
{
    NSMutableDictionary *ret = [[MTLOldFeedItem JSONKeyPathsByPropertyKey] mutableCopy];
...
-(NSArray *)sortedMessages {
    return [_messages sortedArrayUsingComparator:^NSComparisonResult(MTLMessage *message1, MTLOldFeedItem *message2) {
...
+ (void)fetchNewRemoteFeedItemsWithParams:(NSDictionary*)params andBlock:(void (^)(NSArray *))block
{
    [[DockedAPIClient sharedClient] GET:@"feed.json" parameters:params success:^(NSURLSessionDataTask *task, id JSON) {
        ...
        transformer = [NSValueTransformer mtl_JSONArrayTransformerWithModelClass:MTLOldFeedItem.class];
