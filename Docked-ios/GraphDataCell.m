...
- (void) initPlotSpaceWithIndex:(NSNumber *)index
{
    ...
    [plotSpace setYRange: [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat( 0 ) length:CPTDecimalFromFloat( 110 )]];
    ...
    if (plot.identifier == 0) {
        ...
        if(fieldEnum == CPTScatterPlotFieldX)
        {
          ...
          if(fieldEnum == CPTScatterPlotFieldX)
          {
              ...
              // Return y value, for this example we'll be plotting y = x * x
              return secondCoordinates[index];
          }
...
- (void)configureForItem:(FeedItem *)item
{
    ...
    if( cachedValue )
       [graphImageView setImage:cachedValue];
        ...
        [plotSpace setYRange: [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat( 0 ) length:CPTDecimalFromFloat( [graphCardItem.maxYCoordinate floatValue] )]];
